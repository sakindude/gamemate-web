...
function confirmButton(page: Page) {
  return page.locator('aside button').filter({ hasText: 'Confirm Booking' }).first()
}

test('double submit should not create duplicate booking', async ({ page }) => {
  ...
  // tek success olmalı
  await expect(page.getByText('Booking created successfully.')).toHaveCount(1)
