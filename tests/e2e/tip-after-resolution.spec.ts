...
test('buyer_favor blocks tip after dispute resolution', async ({ page }) => {
  runSeed('seed-dispute-resolve-buyer-favor-flow.mjs')
  ...
  const session = await getSessionForBooking(admin, String(booking.id))
  ...
  expect(sessionWithTip.tip_block_reason).toBe('buyer_favor_blocked')
  ...
  const blockedCard = page
    ...
    .filter({ hasText: /tipping blocked after buyer-favor dispute/i })
    .first()

  await expect(blockedCard).toBeVisible({ timeout: 15000 })
  await expect(blockedCard.getByRole('button', { name: /^tip$/i })).toHaveCount(0)
})

test('seller_favor keeps tip available after dispute resolution', async ({ page }) => {
  runSeed('seed-dispute-resolve-seller-favor-flow.mjs')
  ...
  expect(sessionWithTip.tip_eligible).toBe(true)
  expect(sessionWithTip.tip_already_given).toBe(false)
  expect(sessionWithTip.tip_block_reason).toBeNull()

  await login(page, BUYER_EMAIL, BUYER_PASSWORD)
  await openSessions(page)

  const sellerCards = page.locator('article').filter({ hasText: SELLER_NAME })
  await expect(sellerCards.first()).toBeVisible({ timeout: 15000 })

  const tipButtons = sellerCards.getByRole('button', { name: /^tip$/i })
  await expect(tipButtons.first()).toBeVisible({ timeout: 15000 })
})

test('partial resolution keeps tip available after dispute resolution', async ({ page }) => {
  runSeed('seed-dispute-resolve-partial-flow.mjs')
  ...
  expect(session.status).toBe('completed')
  expect(dispute.status).toBe('resolved_partial')
  expect(sessionWithTip.tip_eligible).toBe(true)
  expect(sessionWithTip.tip_already_given).toBe(false)
  expect(sessionWithTip.tip_block_reason).toBeNull()
