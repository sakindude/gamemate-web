...
function countSellerFavorEvents(events) {
  return events.filter((row) => row.event_type === 'dispute_resolved_seller_favor').length
}
...
console.log('--- VERIFY: SELLER FAVOR SINGLE EXECUTION ---')
...
const { data: firstCall, error: firstError } = await admin.rpc('resolve_dispute', {
  ...
  p_resolution_note: 'verify seller favor single execution',
})
...
const { data: secondCall, error: secondError } = await admin.rpc('resolve_dispute', {
  ...
  p_resolution: 'seller_favor',
  p_resolved_by_user_id: ADMIN_USER_ID,
  p_resolution_note: 'verify seller favor single execution second call',
})
...
// 2) dispute must be in resolved seller favor state and remain stable
if (disputeAfter.status !== 'resolved_seller_favor') {
...
// 3) payout must be released once
if (payoutAfter.status !== 'released') {
  throw new Error(`Payout hold status mismatch: ${payoutAfter.status}`)
}

// 4) seller should gain payout once, buyer should gain nothing
...
if (buyerBalanceDiff !== 0) {
  throw new Error(`Buyer balance changed unexpectedly: ${buyerBalanceDiff}`)
}

// 5) wallet rows: exactly one new seller-side payout row, zero buyer refund rows
...
if (buyerWalletDiff !== 0) {
  throw new Error(`Expected 0 new buyer wallet rows, got ${buyerWalletDiff}`)
}
...
if (buyerRefundRowCount !== 0) {
  throw new Error(`Expected 0 buyer refund rows for booking, got ${buyerRefundRowCount}`)
}

// 6) exactly one seller-favor resolution event
const sellerFavorEventCount = countSellerFavorEvents(eventsAfter)
const sellerFavorEventCountBefore = countSellerFavorEvents(eventsBefore)
const sellerFavorEventDiff = sellerFavorEventCount - sellerFavorEventCountBefore

console.log('Seller favor event diff:', sellerFavorEventDiff)

if (sellerFavorEventDiff !== 1) {
  throw new Error(`Expected exactly 1 new seller-favor event, got ${sellerFavorEventDiff}`)
}

// 7) seller payout should match dispute financial fields if present
const sellerAmount = Number(disputeAfter.seller_amount_cents ?? 0)
if (sellerAmount < 0) {
  throw new Error(`Invalid dispute seller_amount_cents: ${sellerAmount}`)
}

if (sellerBalanceDiff !== sellerAmount) {
  throw new Error(
    `Seller balance diff (${sellerBalanceDiff}) does not match dispute seller_amount_cents (${sellerAmount})`
  )
}
