...
async function setBuyerLowBalance(buyerId) {
  const targetBalance = 100

  const { error: profileUpdateError } = await admin
    .from('profiles')
    .update({ balance_cents: targetBalance })
    .eq('id', buyerId)

  failIfError('Failed to set buyer low balance', profileUpdateError)
...
async function run() {
  console.log('--- SEED: BOOKING INSUFFICIENT BALANCE UI ---')
  ...
  const buyerBalance = await setBuyerLowBalance(buyer.id)
  console.log(`Buyer balance forced low: ${buyerBalance}`)
