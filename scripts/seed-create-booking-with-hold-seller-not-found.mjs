...
const NON_EXISTENT_SELLER_ID = '11111111-1111-1111-1111-111111111111'
...
async function run() {
  console.log('--- SEED: CREATE BOOKING WITH HOLD SELLER NOT FOUND ---')
  ...
  console.log('Non-existent seller id:', NON_EXISTENT_SELLER_ID)
  ...
  const { data: rpcResult, error: rpcError } = await buyerAnon.rpc('create_booking_with_hold', {
    p_seller_id: NON_EXISTENT_SELLER_ID,
  ...
  assert(rpcResult.success === false, 'Expected success=false for seller not found')
  assert(rpcResult.message === 'Seller not found.', `Unexpected message: ${rpcResult.message}`)
  assert(!rpcResult.request_id, 'RPC should not return request_id on seller not found')
  ...
  const recentMatchingRows = (recentWalletRows ?? []).filter(
    (row) =>
      ...
      row.direction === 'debit' &&
      row.status === 'posted' &&
      row.metadata?.seller_id === NON_EXISTENT_SELLER_ID &&
  ...
  console.log('Seller not found behavior verified')
  console.log('--- DONE ---')
}
