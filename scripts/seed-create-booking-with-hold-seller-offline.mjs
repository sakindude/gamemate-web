...
async function run() {
  console.log('--- SEED: CREATE BOOKING WITH HOLD SELLER OFFLINE ---')
  ...
  const { error: sellerOfflineError } = await admin
    .from('profiles')
    .update({ is_online: false })
    .eq('id', sellerBefore.id)

  if (sellerOfflineError) throw sellerOfflineError
  console.log('Seller forced offline')
  ...
  assert(rpcResult.success === false, 'Expected success=false for seller offline')
  assert(rpcResult.message === 'Seller is offline.', `Unexpected message: ${rpcResult.message}`)
  assert(!rpcResult.request_id, 'RPC should not return request_id on seller offline')
  ...
  console.log('Seller offline behavior verified')
  console.log('--- DONE ---')
}
