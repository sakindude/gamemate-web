...
async function ensureBuyerHasBalance(profileId, currentBalanceCents, requiredAmountCents) {
  ...
  const { error: walletInsertError } = await admin
    ...
    .insert({
      ...
      currency: 'USD',
      status: 'posted',
      note: 'Seed top-up for seller offline UI test',
      metadata: {
        source: 'seed-booking-seller-offline-ui',
      },
    })
...
async function forceSellerOffline(sellerId) {
  const { error } = await admin
    .from('profiles')
    .update({ is_online: false })
    .eq('id', sellerId)

  failIfError('Failed to force seller offline', error)
}
...
async function verifyStates(buyerId, sellerId) {
  ...
  if (availability?.is_bookable !== false || availability?.reason !== 'offline') {
    fail(`Seller availability is not offline as expected: ${JSON.stringify(availability)}`)
  }
...
async function run() {
  console.log('--- SEED: BOOKING SELLER OFFLINE UI ---')
  ...
  await forceSellerOffline(seller.id)
  ...
  console.log(`Seller availability reason: ${verification.availability.reason}`)
