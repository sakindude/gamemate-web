...
async function ensureBuyerHasBalance(profileId, currentBalanceCents, requiredAmountCents) {
  ...
  const { error: walletInsertError } = await admin
    ...
    .insert({
      ...
      currency: 'USD',
      status: 'posted',
      note: 'Seed top-up for buyer no-show flow',
      metadata: {
        source: 'seed-flow-no-show-buyer',
      },
    })
...
async function createBuyerNoShowReadySession({ buyerId, sellerId, buyerBalanceCents }) {
  ...
  const { data: session, error: sessionInsertError } = await admin
    ...
    .insert({
      ...
      ended_at: null,
      completed_at: null,
      buyer_started_at: null,
      seller_started_at: new Date(now - 35 * 60 * 1000).toISOString(),
  ...
  const { data: payoutHold, error: payoutHoldInsertError } = await admin
    ...
    .insert({
      ...
      refunded_at: null,
      dispute_id: null,
      notes: 'Seeded buyer no-show flow hold',
    })
    .select('*')
    .single()
...
async function run() {
  console.log('--- SEED: FLOW NO SHOW BUYER ---')
  ...
  const { booking, session, payoutHold } = await createBuyerNoShowReadySession({
    buyerId: buyer.id,
    sellerId: seller.id,
