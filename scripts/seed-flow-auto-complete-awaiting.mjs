...
async function ensureBuyerHasBalance(profileId, currentBalanceCents, requiredAmountCents) {
  ...
  const { error: walletInsertError } = await admin
    ...
    .insert({
      ...
      currency: 'USD',
      status: 'posted',
      note: 'Seed top-up for auto-complete flow',
      metadata: {
        source: 'seed-flow-auto-complete-awaiting',
      },
    })
...
async function createAwaitingAutoCompleteFlow({ buyerId, sellerId, buyerBalanceCents }) {
  ...
  const sellerCompletedAt = new Date(now - 27 * 60 * 60 * 1000).toISOString()
  const plannedEndAt = new Date(now - 27 * 60 * 60 * 1000 + 60 * 60 * 1000).toISOString()
  const autoCompleteAt = new Date(now - 2 * 60 * 60 * 1000).toISOString()
  ...
  const releasableAt = new Date(now + 24 * 60 * 60 * 1000).toISOString()
  ...
  const { data: payoutHold, error: payoutHoldInsertError } = await admin
    ...
    .insert({
      ...
      refunded_at: null,
      dispute_id: null,
      notes: 'Seeded auto-complete awaiting flow hold',
    })
    .select('*')
    .single()
...
async function run() {
  console.log('--- SEED: FLOW AUTO COMPLETE AWAITING ---')
  ...
  const { booking, session, payoutHold } = await createAwaitingAutoCompleteFlow({
    buyerId: buyer.id,
    sellerId: seller.id,
  ...
  console.log(`Session auto_complete_at: ${session.auto_complete_at}`)
