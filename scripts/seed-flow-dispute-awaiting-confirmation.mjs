...
async function ensureBuyerHasBalance(profileId, currentBalanceCents, requiredAmountCents) {
  ...
  const { error: walletInsertError } = await admin
    ...
    .insert({
      ...
      currency: 'USD',
      status: 'posted',
      note: 'Seed top-up for dispute flow',
      metadata: {
        source: 'seed-flow-dispute-awaiting-confirmation',
      },
    })
...
async function createAwaitingConfirmationFlow({ buyerId, sellerId, buyerBalanceCents }) {
  ...
  const sellerCompletedAt = new Date(Date.now() - 2 * 60 * 1000).toISOString()
  const autoCompleteAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString()
  const plannedEndAt = new Date(Date.now() + 55 * 60 * 1000).toISOString()
  ...
  const { data: payoutHold, error: payoutHoldInsertError } = await admin
    ...
    .insert({
      ...
      refunded_at: null,
      dispute_id: null,
      notes: 'Seeded dispute awaiting_confirmation flow hold',
    })
    .select('*')
    .single()
...
async function run() {
  console.log('--- SEED: FLOW DISPUTE AWAITING CONFIRMATION ---')
  ...
  const { booking, session, payoutHold } = await createAwaitingConfirmationFlow({
    buyerId: buyer.id,
    sellerId: seller.id,
