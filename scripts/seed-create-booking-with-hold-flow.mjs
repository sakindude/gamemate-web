...
const BUYER_EMAIL = process.env.GM_TEST_BUYER_EMAIL ?? 'gm_test_buyer@example.com'
const BUYER_PASSWORD = process.env.GM_TEST_BUYER_PASSWORD ?? 'gm_test_buyer_123456'
const SELLER_EMAIL = process.env.GM_TEST_SELLER_EMAIL ?? 'gm_test_seller@example.com'
...
async function getLatestWalletHold(bookingId, buyerId) {
  const { data, error } = await admin
    .from('wallet_transactions')
    .select('*')
...
async function run() {
  console.log('--- SEED: CREATE BOOKING WITH HOLD FLOW ---')
  ...
  const { data: buyerBlockingStill, error: buyerBlockingStillError } = await admin
    .from('sessions')
    .select('id, status')
    .eq('buyer_id', buyerBefore.id)
    .in('status', ['ready_to_start', 'active', 'awaiting_confirmation'])

  if (buyerBlockingStillError) throw buyerBlockingStillError
  assert((buyerBlockingStill ?? []).length === 0, 'Buyer still has blocking sessions')

  const { data: sellerBlockingStill, error: sellerBlockingStillError } = await admin
    .from('sessions')
    .select('id, status')
    .eq('seller_id', sellerBefore.id)
    .in('status', ['ready_to_start', 'active', 'awaiting_confirmation'])

  if (sellerBlockingStillError) throw sellerBlockingStillError
  assert((sellerBlockingStill ?? []).length === 0, 'Seller still has blocking sessions')
  ...
  assert(rpcResult?.success === true, 'create_booking_with_hold did not return success=true')

  const bookingRequestId = rpcResult?.request_id
  assert(bookingRequestId, 'RPC did not return request_id')

  console.log('Booking request created:', bookingRequestId)

  const { data: bookingRow, error: bookingRowError } = await admin
    .from('booking_requests')
    .select('*')
  ...
  assert(bookingRow, 'booking_requests row not found')
  assert(bookingRow.buyer_id === buyerBefore.id, 'booking_requests.buyer_id mismatch')
  assert(bookingRow.seller_id === sellerBefore.id, 'booking_requests.seller_id mismatch')
  assert(bookingRow.status === 'pending', `Expected pending booking, got ${bookingRow.status}`)
  assert(bookingRow.base_price_cents === BASE_PRICE_CENTS, 'base_price_cents mismatch')
  assert(bookingRow.tip_cents === TIP_CENTS, 'tip_cents mismatch')
  assert(
    bookingRow.processing_fee_cents === PROCESSING_FEE_CENTS,
    'processing_fee_cents mismatch'
  )
  assert(bookingRow.total_amount_cents === totalAmountCents, 'total_amount_cents mismatch')
  assert(
    bookingRow.communication_method === COMMUNICATION_METHOD,
    'communication_method mismatch'
  )
  assert(bookingRow.game === GAME, 'game mismatch')
  console.log('booking_requests row verified')

  const walletHold = await getLatestWalletHold(bookingRequestId, buyerBefore.id)

  assert(walletHold, 'booking_hold wallet transaction not found')
  assert(walletHold.direction === 'debit', 'wallet hold direction mismatch')
  assert(walletHold.amount_cents === totalAmountCents, 'wallet hold amount mismatch')
  assert(walletHold.status === 'posted', 'wallet hold status mismatch')
  console.log('wallet_transactions booking_hold row verified')

  const buyerAfter = await getProfileByEmail(BUYER_EMAIL)
  const buyerBalanceAfter = buyerAfter.balance_cents ?? 0
  const expectedAfterBalance = buyerBalanceBefore - totalAmountCents

  console.log('Buyer balance before:', buyerBalanceBefore)
  console.log('Buyer balance after:', buyerBalanceAfter)
  console.log('Expected buyer balance after:', expectedAfterBalance)

  assert(
    buyerBalanceAfter === expectedAfterBalance,
    `Buyer balance reserve mismatch. Expected ${expectedAfterBalance}, got ${buyerBalanceAfter}`
  )

  console.log('Buyer balance reserve effect verified')
  console.log('--- DONE ---')
}
