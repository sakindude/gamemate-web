...
async function forceBookingOldEnough(bookingId) {
  ...
  if (error) fail(`forceBookingOldEnough failed: ${error.message}`)
  if (!data) fail('forceBookingOldEnough returned no row')
...
async function callTimeout(bookingId, label, delayMs = 0) {
  if (delayMs > 0) {
    await new Promise((resolve) => setTimeout(resolve, delayMs))
  }

  const { data, error } = await admin.rpc('update_booking_request_status_with_refund', {
    p_request_id: bookingId,
    p_status: 'expired',
  })
...
async function assertNoOrphanState(buyerId, bookingId) {
  ...
  assert(booking, `Booking ${bookingId} missing after overlap flow`)
...
async function assertOverlapResult(bookingId, buyerId, buyerBalanceBeforeRefund, results) {
  ...
  assert(successLike.length >= 1, 'At least one overlap path should complete successfully')
  assert(failureLike.length <= 1, 'At most one overlap path should fail/no-op')

  assert(
    bookingAfter.status === 'rejected' || bookingAfter.status === 'expired',
    `Booking final status must be rejected or expired, got ${bookingAfter.status}`
  )

  const refundCount = countRefundRows(buyerWalletRows)
  assertEqual(refundCount, 1, 'refund row count after timeout/reject overlap')

  assertEqual(
    buyerBalanceAfter,
    buyerBalanceBeforeRefund + TOTAL_AMOUNT_CENTS,
    'buyer balance after timeout/reject overlap'
  )
...
async function main() {
  ...
  const allowed = new Set([
    'timeout_vs_reject_parallel',
    'reject_then_timeout',
    'timeout_then_reject',
  ])

  if (!scenario) {
    fail('Missing --scenario. Supported: timeout_vs_reject_parallel, reject_then_timeout, timeout_then_reject')
  }
  ...
  if (!SELLER_EMAIL || !SELLER_PASSWORD) fail('Missing seller credentials from PW_TEST_SELLER_EMAIL / PW_TEST_SELLER_PASSWORD / PW_TEST_PASSWORD')

  console.log(`--- SEED: TIMEOUT VS REJECT CONCURRENCY (${scenario.toUpperCase()}) ---`)
  ...
  const sellerClient = createAnonClient()
  ...
  try {
    ...
    const forced = await forceBookingOldEnough(bookingId)
    console.log('Booking forced old enough for timeout:', forced.created_at)
    ...
    await signInSeller(sellerClient)
    console.log('Seller client authenticated')

    let results = []

    if (scenario === 'timeout_vs_reject_parallel') {
      results = await Promise.all([
        callReject(sellerClient, bookingId, 'reject-call'),
        callTimeout(bookingId, 'timeout-call'),
      ])
    }

    if (scenario === 'reject_then_timeout') {
      const first = await callReject(sellerClient, bookingId, 'reject-first-call')
      const second = await callTimeout(bookingId, 'timeout-second-call', 25)
      results = [first, second]
    }

    if (scenario === 'timeout_then_reject') {
      const first = await callTimeout(bookingId, 'timeout-first-call')
      const second = await callReject(sellerClient, bookingId, 'reject-second-call', 25)
      results = [first, second]
    }

    logResults(results)
    await assertOverlapResult(bookingId, buyer.id, buyerBalanceBeforeRefund, results)
