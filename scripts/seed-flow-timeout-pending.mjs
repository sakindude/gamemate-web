...
async function ensureBuyerHasBalance(profileId, currentBalanceCents, requiredAmountCents) {
  ...
  const { error: walletInsertError } = await admin
    ...
    .insert({
      ...
      currency: 'USD',
      status: 'posted',
      note: 'Seed top-up for timeout pending flow',
      metadata: {
        source: 'seed-flow-timeout-pending',
      },
    })
...
async function createExpiredPendingBookingAndHold({ buyerId, sellerId, buyerBalanceCents }) {
  ...
  const expiredCreatedAt = new Date(Date.now() - 11 * 60 * 1000).toISOString()
  ...
  failIfError('Expired pending booking insert failed', bookingInsertError)

  if (!booking?.id) {
    fail('Expired pending booking insert returned no row')
  }
  ...
  const { data: payoutHold, error: payoutHoldInsertError } = await admin
    ...
    .insert({
      ...
      status: 'held',
      held_at: expiredCreatedAt,
      notes: 'Seeded expired pending timeout flow hold',
    })
    .select('*')
    .single()
...
async function run() {
  console.log('--- SEED: FLOW TIMEOUT PENDING ---')
  ...
  const { booking, payoutHold } = await createExpiredPendingBookingAndHold({
    buyerId: buyer.id,
    sellerId: seller.id,
  ...
  console.log(`Pending created_at: ${booking.created_at}`)
