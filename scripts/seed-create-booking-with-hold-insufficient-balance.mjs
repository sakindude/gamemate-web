...
async function getWalletRowsForBooking(bookingId, buyerId) {
  const { data, error } = await admin
    .from('wallet_transactions')
    .select('*')
...
async function run() {
  console.log('--- SEED: CREATE BOOKING WITH HOLD INSUFFICIENT BALANCE ---')
  ...
  const forcedBuyerBalance = totalAmountCents - 1
  ...
  const { error: buyerBalanceSetError } = await admin
    .from('profiles')
    .update({ balance_cents: forcedBuyerBalance })
    .eq('id', buyerBefore.id)

  if (buyerBalanceSetError) throw buyerBalanceSetError

  console.log('Buyer balance forced to:', forcedBuyerBalance)
  console.log('Required total_amount_cents:', totalAmountCents)
  ...
  assert(rpcResult.success === false, 'Expected success=false for insufficient balance')
  assert(rpcResult.message === 'Insufficient balance.', `Unexpected message: ${rpcResult.message}`)

  const unexpectedRequestId = rpcResult?.request_id ?? null
  assert(!unexpectedRequestId, 'RPC should not return request_id on insufficient balance')

  const buyerAfter = await getProfileByEmail(BUYER_EMAIL)
  const buyerBalanceAfter = buyerAfter.balance_cents ?? 0

  console.log('Buyer balance before RPC:', forcedBuyerBalance)
  console.log('Buyer balance after RPC:', buyerBalanceAfter)

  assert(
    buyerBalanceAfter === forcedBuyerBalance,
    `Buyer balance changed unexpectedly. Expected ${forcedBuyerBalance}, got ${buyerBalanceAfter}`
  )
  ...
  assert((pendingRows ?? []).length === 0, 'Unexpected pending booking was created')

  if (unexpectedRequestId) {
    const bookingRow = await getBookingById(unexpectedRequestId)
    assert(!bookingRow, 'Unexpected booking row exists for returned request_id')

    const walletRows = await getWalletRowsForBooking(unexpectedRequestId, buyerBefore.id)
    assert(walletRows.length === 0, 'Unexpected wallet rows exist for returned request_id')
  }
  ...
  console.log('Insufficient balance behavior verified')
  console.log('--- DONE ---')
}
