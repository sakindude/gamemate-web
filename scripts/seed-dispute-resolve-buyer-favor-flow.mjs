...
async function run() {
  console.log('--- SEED: DISPUTE RESOLVE (BUYER FAVOR) ---')
  ...
  const { data: dispute, error: disputeError } = await supabase
    ...
    .insert({
      ...
      description: 'Seed dispute for buyer favor resolve flow',
      evidence: {},
      status: 'open',
  ...
  const { data: linkedHold, error: linkedHoldError } = await supabase
    ...
    .update({
      ...
      notes: 'Seed-linked dispute for buyer favor resolve flow',
      status: 'disputed',
    })
    .eq('id', hold.id)
    .select('*')
  ...
  const { data: resolvedDispute, error: resolveError } = await supabase.rpc('resolve_dispute', {
    ...
    p_resolution_note: 'Seed buyer favor resolve test',
  })
  ...
  const buyerWalletCountAfter = buyerWalletBeforeRows
    ? buyerWalletCountBefore + 1 <= (buyerWalletRows?.length ?? 0)
      ? buyerWalletRows.length
      : buyerWalletCountBefore
    : buyerWalletRows?.length ?? 0

  const sellerWalletCountAfter = sellerWalletRows?.length ?? 0

  if (finalDispute.status !== 'resolved_buyer_favor') {
    throw new Error(`Expected resolved_buyer_favor but got ${finalDispute.status}`)
  }

  if (payoutHoldAfter.status !== 'refunded') {
    throw new Error(`Expected refunded but got ${payoutHoldAfter.status}`)
  }
  ...
  if (!buyerRefundRow) {
    throw new Error('Buyer refund row not found')
  }

  if (sellerWalletCountAfter > 0) {
    throw new Error('Seller payout should not happen for buyer favor flow')
  }
  ...
  if (sellerBalanceAfter !== sellerBalanceBefore) {
    throw new Error('Seller balance should not change for buyer favor flow')
  }

  console.log('Buyer wallet count before:', buyerWalletCountBefore)
  console.log('Buyer wallet count after (booking rows only):', buyerWalletRows?.length ?? 0)
