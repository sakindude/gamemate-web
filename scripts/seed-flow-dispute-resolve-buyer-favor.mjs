...
async function ensureBuyerHasBalance(profileId, currentBalanceCents, requiredAmountCents) {
  ...
  const { error: walletInsertError } = await admin
    ...
    .insert({
      ...
      currency: 'USD',
      status: 'posted',
      note: 'Seed top-up for dispute resolve buyer favor flow',
      metadata: {
        source: 'seed-flow-dispute-resolve-buyer-favor',
      },
    })
...
async function createDisputedFlow({ buyerId, sellerId, buyerBalanceCents }) {
  ...
  const { data: dispute, error: disputeInsertError } = await admin
    ...
    .insert({
      ...
      description: 'Seeded dispute resolution test - buyer favor path',
  ...
  const { data: payoutHold, error: payoutHoldInsertError } = await admin
    ...
    .insert({
      ...
      notes: 'Seeded dispute resolve buyer favor flow hold',
    })
    .select('*')
    .single()
...
async function run() {
  console.log('--- SEED: FLOW DISPUTE RESOLVE BUYER FAVOR ---')
