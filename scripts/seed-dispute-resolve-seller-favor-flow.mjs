...
async function run() {
  console.log('--- SEED: DISPUTE RESOLVE (SELLER FAVOR) ---')
  ...
  const { data: dispute, error: disputeError } = await supabase
    ...
    .insert({
      ...
      description: 'Seed dispute for seller favor resolve flow',
      evidence: {},
      status: 'open',
  ...
  const { data: linkedHold, error: linkedHoldError } = await supabase
    ...
    .update({
      ...
      notes: 'Seed-linked dispute for seller favor resolve flow',
      status: 'disputed',
    })
    .eq('id', hold.id)
    .select('*')
  ...
  const { data: resolvedDispute, error: resolveError } = await supabase.rpc('resolve_dispute', {
    ...
    p_resolution_note: 'Seed seller favor resolve test',
  })
  ...
  if (finalDispute.status !== 'resolved_seller_favor') {
    throw new Error(`Expected resolved_seller_favor but got ${finalDispute.status}`)
  }

  if (payoutHoldAfter.status !== 'released') {
    throw new Error(`Expected released but got ${payoutHoldAfter.status}`)
  }
  ...
  if (!sellerPayoutRow) {
    throw new Error('Seller payout row not found')
  }

  if (buyerWalletCountAfter > 0) {
    throw new Error('Buyer refund should not happen for seller favor flow')
  }
  ...
  if (buyerBalanceAfter !== buyerBalanceBefore) {
    throw new Error('Buyer balance should not change for seller favor flow')
  }
