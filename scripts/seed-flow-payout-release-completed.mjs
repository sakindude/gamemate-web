...
async function setSellerPayoutEligibilityApproved(sellerId) {
  const { data, error } = await admin.rpc('gm_admin_set_seller_payout_eligibility', {
    p_user_id: sellerId,
    p_status: 'approved',
    p_note: 'Seed setup for payout release success flow',
  })

  failIfError('Seller payout eligibility approval failed', error)

  if (data?.success === false) {
    fail(data?.message || 'Seller payout eligibility approval returned success=false')
  }
}
...
async function ensureBuyerHasBalance(profileId, currentBalanceCents, requiredAmountCents) {
  ...
  const { error: walletInsertError } = await admin
    ...
    .insert({
      ...
      currency: 'USD',
      status: 'posted',
      note: 'Seed top-up for payout release flow',
      metadata: {
        source: 'seed-flow-payout-release-completed',
      },
    })
...
async function createCompletedFlow({ buyerId, sellerId, buyerBalanceCents }) {
  ...
  const completedAt = new Date(now - 50 * 60 * 1000).toISOString()
  const plannedEndAt = new Date(now - 30 * 60 * 1000).toISOString()
  const releasableAt = new Date(now - 25 * 60 * 60 * 1000).toISOString()
  const disputeDeadlineAt = new Date(now - 24 * 60 * 60 * 1000).toISOString()
  ...
  const { data: session, error: sessionInsertError } = await admin
    ...
    .insert({
      ...
      buyer_completed_at: completedAt,
  ...
  const { data: payoutHold, error: payoutHoldInsertError } = await admin
    ...
    .insert({
      ...
      refunded_at: null,
      dispute_id: null,
      notes: 'Seeded payout release completed flow hold',
    })
    .select('*')
    .single()
  ...
  return { booking, session, payoutHold, sellerPayoutCents }
}

async function run() {
  console.log('--- SEED: FLOW PAYOUT RELEASE COMPLETED ---')
  ...
  await setSellerPayoutEligibilityApproved(seller.id)
  console.log('Seller payout eligibility forced to approved')
  ...
  const { booking, session, payoutHold, sellerPayoutCents } = await createCompletedFlow({
    buyerId: buyer.id,
    sellerId: seller.id,
  ...
  console.log(`Payout releasable_at: ${payoutHold.releasable_at}`)
  console.log(`Expected seller payout cents: ${sellerPayoutCents}`)
  console.log('--- DONE ---')
}
